...
// A TOUCH session can never consume the verdict (steadySixtyDesktopProfile
// requires maxTouchPoints === 0), so it must not pay the streak bookkeeping to
// build one.
describe("steadySixtyCadence touch guard", () => {
  ...
  it("does not accumulate the verdict", () => {
    withTouch(5, () => {
      reportInFlightCadence(16.7);
      reportInFlightCadence(16.7);
      expect(steadySixtyVerified()).toBe(false);
