 ...
 * press does. Committing through the navigation controller instead would move
 * the store ahead of the browser and mark a guard the sync would then eat.
 *
 ...
 * lands on it. There the commit has to drive the stores directly, so it calls
 * `pop` — the binding's own navigation surface, the one the back control uses.
 * The driver stays in step because `pop` calls `back()` itself.
 */
export default function commitScopeBack(stores: FlemoStores, pop: () => void): void {
  if (!stores.memory) {
    stores.driver.back();
    return;
  }
