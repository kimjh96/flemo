...
describe("buildRoutePath", () => {
  it("compiles path params and appends extra params as search", () => {
    const result = buildRoutePath("/users/:id", { id: 42, ref: "home" });
  ...
  it("returns pathname unchanged when there are no extra params", () => {
    const result = buildRoutePath("/about", {});
  ...
  it("stringifies non-string param values", () => {
    const result = buildRoutePath("/posts/:slug", { slug: "hello", page: 2 });

    expect(result.pathname).toBe("/posts/hello?page=2");
  });

  it("fills multiple path params in order", () => {
    const result = buildRoutePath("/users/:userId/posts/:postId", { userId: 42, postId: "abc" });
  ...
  it("handles undefined params by falling back to an empty object", () => {
    const result = buildRoutePath("/about", undefined);
