...
const shiftAnimations = (element: HTMLElement, excessMs: number, subtree: boolean) => {
  if (typeof element.getAnimations !== "function") return;
  for (const animation of element.getAnimations({ subtree })) {
...
// re-anchors every flemo animation on the given elements (the first element's
// SUBTREE is included — parts live inside the screen scope). `onStall` fires
// after a shift so the engine can push its own wall-clock deadlines (the
// recovery watchdog, the perceptual cut) out of the way. Returns a detach.
    ...
    if (last !== null) {
      ...
      if (gap > NATIVE_STALL_STEP_MS) {
        ...
        targets.forEach((element, index) => {
          if (!element) return;
          shiftAnimations(element, excess, index === 0);
        });
        onStall?.(excess);
      }
    }
