...
describe("driverPolicy.pinned (shell-first takeover override)", () => {
  it("is true only while a valid force key is set, read live", () => {
    const policy = createDriverPolicy(memoryStorage().storage, true);
    localStorage.removeItem("flemo:motion-driver-force");
    expect(policy.pinned()).toBe(false);

    localStorage.setItem("flemo:motion-driver-force", "raf");
    expect(policy.pinned()).toBe(true); // the pin overrides the takeover
    localStorage.setItem("flemo:motion-driver-force", "css");
    expect(policy.pinned()).toBe(true); // either direction counts as pinned
    localStorage.setItem("flemo:motion-driver-force", "garbage");
    expect(policy.pinned()).toBe(false); // an invalid value is no pin
    localStorage.removeItem("flemo:motion-driver-force");
    expect(policy.pinned()).toBe(false);
  });

  it("does not warn (a pure read, unlike playerAllowed)", () => {
    const policy = createDriverPolicy(memoryStorage().storage, true);
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    localStorage.setItem("flemo:motion-driver-force", "raf");
    policy.pinned();
