...
<Cards>
  <Card title="cupertino" description="iOS 스타일 가로 슬라이드 — 엣지 드래그 백 포함, 기본값" />
  <Card title="material" description="아래에서 위로 슬라이드 — 드래그 dismiss + 러버밴드 저항" />
...
흔한 형태는 `createTransition`(여섯 단계), 모든 navigate 상태를 분리해서 정의하고 싶다면
`createRawTransition`을 써요.
...
받아요. 제스처 도중에 요소를 움직이는 책임이 핸들러에 있어요.

## 데코레이터

데코레이터는 이전 화면과 현재 화면 사이에 끼는 레이어예요. 내장 `overlay` 데코레이터가
...
const dim = createDecorator({
  ...
  // 머물러요. dim이 보이면 안 되는 슬롯들이라 0으로 잡아요.
...
상태별 완전 제어가 필요하면 `createRawDecorator`가 `createRawTransition`과 같은 형태로
제공돼요.
