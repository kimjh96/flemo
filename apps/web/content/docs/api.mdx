...
| `LayoutScreen` | Screen variant for shared `layoutId` morphs across screens.        | `@flemo/layout` | [LayoutScreen](./layout-screen) |
| `LayoutConfig` | Wraps motion children with the current screen's transition timing. | `@flemo/layout` | [LayoutScreen](./layout-screen) |
...
`@flemo/react` requires only `react ^19` and `react-dom ^19`. `@flemo/layout` adds `motion ^12`
