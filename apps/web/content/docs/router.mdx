...
`useStores()` is the entry point: it returns all four request-scoped store APIs (`history`,
`navigate`, `transition`, `screen`) for imperative `getState()` / `subscribe` — what most
outside-the-Router code wants.

For reactive reads, three lower-level selector hooks subscribe to a single store and re-render on
change, like a zustand hook:
