...
```text
                ┌─ skip: 2  ≡  until: "/library"   (the target)
                ▼
   /    →    /library    →    /posts/a    →    /posts/b
  (0)         (1)              (2)              (3) ◀ top

  pop      →  /, /library             land on the target
  replace  →  /, /x                   replace the target and everything above it
  push     →  /, /library, /x         keep the target, stack the new screen on top
```
