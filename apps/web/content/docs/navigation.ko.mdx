...
```text
                ┌─ skip: 2  ≡  until: "/library"   (도달 대상)
                ▼
   /    →    /library    →    /posts/a    →    /posts/b
  (0)         (1)              (2)              (3) ◀ top

  pop      →  /, /library             대상에 안착
  replace  →  /, /x                   대상과 그 위 전부를 교체
  push     →  /, /library, /x         대상은 남기고 새 화면을 위에 쌓음
```
...
`skip`·`until`은 `push`·`replace`·`pop` 모두에서 받고, `transitionName`·`layoutId`는
`push`·`replace`에 적용돼요.
