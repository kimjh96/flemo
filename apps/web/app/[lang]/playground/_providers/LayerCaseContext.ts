"use client";

import { createContext, useContext } from "react";

// The fixture's state, held ABOVE both Routers.
//
// Not a detail. The previous fixture kept these in the screen's own useState,
// so pushing mounted a fresh screen with the sheet closed and the toggle back
// at its default — which meant the one arrangement the fixture existed to show
// (a sheet that is open WHILE a screen moves) could not be reached by using
// it. State that survives navigation is the fixture.
export interface LayerCase {
  /**
   * WHICH screen has the sheet up, not merely whether one does. An overlay
   * belongs to one screen; a shared boolean would put an identical sheet on
   * both of them and quietly make "does it travel with its screen" a question
   * about whichever copy the query happened to find first.
   */
  openOn: "A" | "B" | null;
  setOpenOn: (step: "A" | "B" | null) => void;
  /**
   * The single variable. `true` wraps the sheet in <Layer>; `false` writes the
   * identical markup straight into the screen. Nothing else differs between
   * the two runs, which is what makes the difference attributable.
   */
  hosted: boolean;
  setHosted: (hosted: boolean) => void;
}

const LayerCaseContext = createContext<LayerCase | null>(null);

export function useLayerCase(): LayerCase {
  const value = useContext(LayerCaseContext);
  if (!value) throw new Error("useLayerCase must be used inside the layer fixture");
  return value;
}

export default LayerCaseContext;
