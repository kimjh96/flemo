        ...
        <ul className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
          {ACTS.map((act) => (
            <li key={act.id}>
              ...
              >
                {/* NOT a container, and that is a device-tested verdict this
                    page briefly forgot. A row is [thumb | name | price] laid
                    ACROSS; the page is laid DOWN. Pairing the row as a card
                    balloons the arriving page out of it as one huge gradient
                    with the name crossing over it, which the deleted
                    playground recorded as broken "on every transition". From
                    this tab the artwork alone is the shared element; the
                    container transform belongs to the poster grid, whose
                    cells share the page's arrangement. */}
                <Morph
                  as="span"
