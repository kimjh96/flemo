...
| A growing box whose contents stay put is always resized | It is laid out once at the larger end and clipped into view, but only when it clips its overflow and every computed property of the arrival and every carried paint channel of the departure is proven not to paint against the box. A shadow, border, outline, image, mask, filter, transform, percentage corner, ellipsis, pseudo-element, or unknown property lays it out every frame instead. | `morph/morphReveal.ts` |
...
| Screen stacking still applies to a shared element | It cannot. The flight layer is a sibling of every screen container and paints above them. Nothing inside a scope can outrank a flight. Chrome that must stay in front belongs outside the scope; today only a lifted `Part` is. | `react/src/Router.tsx`, `dom/stacking.ts` |
...
Parts and decorators differ in one slot and in how they are reached. Without `dismiss`, `idle` / `enter` / `exit` fade the returning part in on pop while the dismissed part stays fully opaque. The previous workaround required restating all ten variants through `createRawPartTransition`. A decorator has no `dismiss`: it dresses one transition, and the dismissed screen is not the screen it dresses.
