...
- ONLY A MORPH INHERITS A CURVE, and only because it LEFT the screen. A part and a decorator stay inside theirs and ride its transform already, so there is no gap with the screen for a shared curve to close; a morph is absolutely positioned in the flight layer and has to reproduce that screen's motion in its own animation, which is why `attachMorph` gates the rule on `screenMoves` rather than applying it always. Length is a separate question with a separate answer: an omitted duration resolved to ZERO and the part snapped, so inheriting it fixes a broken default, while an omitted curve resolves to CSS `ease`, which is a working one.
